...
import { getActiveChatTools, formatChatError, buildUserChatMessage } from "./chatToolUtils";
...
export function useChatSession({ visible }: { visible: boolean }) {
  ...
  const handleSendChatMessage = async (
    ...
    const activeTools = getActiveChatTools(enabledTools, config);
    ...
    try {
      ...
      if (controller.signal.aborted) {
        ...
        const { isAbort, friendlyMessage } = formatChatError(err, t);
        if (isAbort) {
          console.log("Chat stream aborted by user");
        } else {
          console.error(err);
