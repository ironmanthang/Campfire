    ...
    <div className="pt-4 border-t border-border-brand/40 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-[10px] text-accent-brand hover:underline flex items-center gap-1 cursor-pointer font-bold"
        >
      ...
      <div className="flex flex-wrap gap-1.5 border-b border-border-brand/20 pb-2">
        {(["summary", "nvidia", "ollama", "ps"] as const).map((tab) => (
          <button
            ...
            className={`px-2.5 py-1 rounded-md text-[10px] font-bold border transition-all duration-150 cursor-pointer ${
      ...
      {activeDiagnosticsTab === "summary" && (
        <div className="space-y-4 pt-1">
          ...
          <div className="space-y-2">
            <span className="block text-[10px] font-bold text-text-secondary uppercase tracking-wider">{t("settingsView.loadedModels")}</span>
                  ...
                  <div key={am.name} className="p-2.5 rounded-lg border border-border-brand bg-bg-app text-xs flex justify-between items-center gap-3">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <button
                        ...
                        className="px-2 py-1 rounded bg-bg-input hover:bg-red-500/15 border border-border-brand/60 hover:border-red-500/30 text-red-400 hover:text-red-300 transition-all duration-150 flex items-center gap-1 cursor-pointer font-bold text-[10px] disabled:opacity-50"
          ...
          <div className="space-y-3 border-t border-border-brand/20 pt-3">
            <span className="block text-[10px] font-bold text-text-secondary uppercase tracking-wider">{t("settingsView.hardwareStats")}</span>
            {!resources ? (
              <div className="flex items-center justify-center gap-2 py-3 text-xs text-text-secondary italic">
                <Loader2 className="h-3.5 w-3.5 animate-spin text-accent-brand" />
      ...
      {activeDiagnosticsTab === "nvidia" && (
        <div className="pt-1">
          <span className="block text-[10px] font-bold text-text-secondary uppercase tracking-wider mb-1.5">{t("settingsView.rawNvidiaOutput")}</span>
          <pre className="bg-bg-app border border-border-brand/60 p-3 rounded-lg text-[10px] font-mono whitespace-pre overflow-x-auto text-text-primary max-h-[300px] scrollbar-thin">
      ...
      {activeDiagnosticsTab === "ollama" && (
        <div className="pt-1">
          <span className="block text-[10px] font-bold text-text-secondary uppercase tracking-wider mb-1.5">{t("settingsView.rawOllamaOutput")}</span>
          <pre className="bg-bg-app border border-border-brand/60 p-3 rounded-lg text-[10px] font-mono whitespace-pre overflow-x-auto text-text-primary max-h-[300px] scrollbar-thin">
      ...
      {activeDiagnosticsTab === "ps" && (
        <div className="pt-1">
          <span className="block text-[10px] font-bold text-text-secondary uppercase tracking-wider mb-1.5">{t("settingsView.rawOsOutput")}</span>
          <pre className="bg-bg-app border border-border-brand/60 p-3 rounded-lg text-[10px] font-mono whitespace-pre overflow-x-auto text-text-primary max-h-[300px] scrollbar-thin">
            {resources 
