...
export function ClearCompletedModal({
  ...
  return (
    ...
    >
      <div
        ref={modalRef}
        className="bg-bg-surface border border-border-brand rounded-2xl p-5 max-w-sm w-full shadow-2xl space-y-4 max-h-[85vh] flex flex-col"
      >
        {/* Header */}
        <div className="flex items-start justify-between gap-3 shrink-0">
          ...
          {completedItems.length > 1 && (
            <button
              ...
              className="text-xs font-semibold text-accent-brand active:underline cursor-pointer select-none"
            >
              {isAllSelected
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 border border-border-brand/40 bg-bg-app/40 rounded-xl p-2.5 max-h-56">
            ...
            return (
              <div
                ...
                className={`flex items-start gap-2.5 p-2 rounded-lg cursor-pointer transition-colors select-none ${
                  isSelected
                    ? 'bg-bg-surface border border-border-brand active:border-accent-brand/50'
                    : 'bg-bg-surface/50 border border-border-brand/30 active:border-border-brand'
                }`}
              >
        ...
        <div className="flex items-center justify-end gap-2.5 pt-2 shrink-0 border-t border-border-brand/30">
          <button
            type="button"
            onClick={onCancel}
            className="px-3.5 py-2 rounded-xl border border-border-brand active:border-accent-brand text-xs font-semibold text-text-primary active:bg-bg-surface/50 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-semibold shadow-sm transition-all cursor-pointer disabled:opacity-50 disabled:pointer-events-none ${
              selectedIds.size > 0
                ? 'bg-red-600 active:bg-red-700 text-white'
                : 'bg-accent-brand active:bg-accent-brand/90 text-bg-surface'
            }`}
          >
            {selectedIds.size > 0
